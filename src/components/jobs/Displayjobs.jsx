...
const Displayjobs = () => {
    ...
    useEffect(()=>{
        const fetchjobs=async()=> {
            const res=await fetch(`${jobapi}/displayjobs`,{
                method:"GET",
                header: {
  ...
  return (
    <div>
      <Navbar/>
      <p className='h-10 px-28 mt-10 mx-auto text-xl font-bold'>Search Results: 6...</p>
      <div className="container mx-auto mt-10 h-screen grid grid-cols-3 gap-10 p-10 px-22">
        {
            jobs.map((job)=>(
                ...
                <div className="namer flex items-center gap-4 mt-3 ml-4">
                    ...
                    <img className='' src='https://upload.wikimedia.org/wikipedia/commons/thumb/c/c1/Google_%22G%22_logo.svg/768px-Google_%22G%22_logo.svg.png' width={32}></img></div>
