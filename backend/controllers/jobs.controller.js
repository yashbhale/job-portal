...
import {sendJobEmail} from "../utils/sendEmail.js"
...
export const createjob = async (req, res) => {
  try {
    ...
    const companyj = await company.findById(compid);
    if (!companyj) {
    ...
    if (emails.length > 0) {
      sendJobEmail(emails, newjob, companyj.name)
        .catch(err => console.error("Email error:", err.message));
    }

    return res.status(201).json({
      message: "Job created successfully & notifications sent",
      success: true,
    });
